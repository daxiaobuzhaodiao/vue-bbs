...
function showError(el, error) {
    ...
    if (error === undefined) {
        ...
        parentElement.classList.remove('has-error') // 删除节点类名
    } else {
        errorElement.textContent = error
        errorElement.style.display = 'block'
        parentElement.classList.add('has-error')    // 节点添加类名
    }
}
...
export default {
    ...
    inserted(el, binding, vnode) {
        ...
        // 指定当前一系列验证项的父级， 这里指定为带有 data-validator-form 的元素
        const form = el.closest('[data-validator-form]')
        // 指定一个按钮来检查所有的验证项， 这里指定为带有 type=submit 的元素
